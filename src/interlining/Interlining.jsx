...
export default function InterliningCanvas() {
  ...
  const {
    ...
    options, setOptions,
    handleMouseDown, handleMouseMove, handleMouseUpOrLeave, handleRightClick,
    drawCanvas,
  ...
  const toggles = [
    { value: "showGrid", label: "Toggle Grid", iconOn: <GridOn />, iconOff: <GridOff /> }
  ];
  ...
  return (
    <PageStack>
      ...
      <Stack direction="row" spacing={2} divider={<Divider orientation="vertical" flexItem />}>
        ...
        <ToggleButtonGroup value={options} onChange={(e, v) => setOptions(v)}>
          {toggles.map(t => (
            <Tooltip title={t.label} key={t.value}>
              <ToggleButton value={t.value}>{options.includes(t.value) ? t.iconOn : t.iconOff}</ToggleButton>
            </Tooltip>
          ))}
        </ToggleButtonGroup>
